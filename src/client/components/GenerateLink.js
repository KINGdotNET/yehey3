...
class Index extends React.Component {
  ...
  componentWillMount() {
    const { operation } = this.state.query;
    if (operation) {
  ...
  render() {
    ...
    const { activeSelect, step, stepNumber, filter, operation, query } = this.state;
              ...
              return (
                <Form.Item label={`${field}${isRequired ? '*' : ''}`} key={`f_${field}`}>
                  {getFieldDecorator(field, {
                    rules: [{
                      required: isRequired, message: `${field} ${intl.formatMessage({ id: 'is_required' })}`,
                    }],
                    initialValue:
                    query[field] ||
        ...
        {this.state.step === 'link' && <div className="FormGenerateLinkResult-container">
          <div className="FormGenerateLinkResult-bg">
            <Form className="FormGenerateLinkResult">
              <img src="/images/sign/link-icon.svg" id="link-icon" />
