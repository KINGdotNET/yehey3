...
@connect(
    (state) => ({
      ...
      allAccounts: getAllAccountsList(state).sort(() => 0.5 - Math.random()),
...
class DiscoverRandom extends React.Component {
    static propTypes = {
  ...
  handleLoadMore = () => {
    ...
    const moreUsers = allAccounts.slice(moreUsersStartIndex, moreUsersStartIndex + displayLimit);
    this.setState({
