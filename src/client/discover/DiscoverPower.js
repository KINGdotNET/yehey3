...
class DiscoverPower extends React.Component {
    static propTypes = {
  ...
  componentDidMount() {
    ...
    const initialUsers = allAccounts.sort((a,b) => parseFloat(b.SCORE) - parseFloat(a.SCORE)).slice(0, displayLimit);
    this.setState({
        users: initialUsers,
      });
  };
  ...
  handleLoadMore = () => {
    ...
    const moreUsers = allAccounts.sort((a,b) => parseFloat(b.SCORE) - parseFloat(a.SCORE)).slice(moreUsersStartIndex, moreUsersStartIndex + displayLimit);
    this.setState({
